...
class SingleLayerNN(object):
    ...
    def initialize_weights(self,seed=None):
        ...
        self.weights = np.random.randn(self.number_of_nodes, self.input_dimensions + 1) # adding 1 to dimension to account for biases column
    ...
    def predict(self, X):
        ...
        predictions =  self.weights.dot(np.vstack((np.ones(X.shape[1]),X.copy())))
    ...
    def train(self, X, Y, num_epochs=10,  alpha=0.1):
        ...
        for i in range(num_epochs):
            ...
            X_T = X.T.copy()
            X_T = np.hstack((np.ones([X.shape[1], 1], dtype = int), X_T))
            correction = alpha * errors.dot(X_T)
    ...
    def calculate_percent_error(self,X, Y):
        ...
        for i in range(predictions.shape[1]):
            if  not (np.array_equal(P_T[i], Y_T[i])):
                sum_error += 1
        return 100* (sum_error/predictions.shape[1])
